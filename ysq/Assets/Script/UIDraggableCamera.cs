...
public class UIDraggableCamera : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (this.rootForBounds == null)
		{
			...
			{
				NGUITools.GetHierarchy(base.gameObject) + " needs the 'Root For Bounds' parameter to be set",
				this
			});
			base.enabled = false;
		}
	...
	private void Update()
	{
		...
		else
		{
			...
			{
				this.mTrans.localPosition += (Vector3)NGUIMath.SpringDampen(ref mMomentum, 9f, deltaTime);
				this.mBounds = NGUIMath.CalculateAbsoluteWidgetBounds(this.rootForBounds);
