	...
	public void UpdateGeometry(int widgetCount)
	{
		...
		if (size > 0 && size == this.uvs.size && size == this.cols.size && size % 4 == 0)
		{
			...
			else
			{
				...
				global::Debug.LogError(new object[]
				{
					"Too many vertices on one panel: " + this.verts.size
				});
			}
			if (this.mRenderer == null)
			{
		...
		else
		{
			...
			global::Debug.LogError(new object[]
			{
				"UIWidgets must fill the buffer with 4 vertices per quad. Found " + size
			});
		}
		this.verts.Clear();
		this.uvs.Clear();
				...
				if (parentPanel.hasClipping)
				{
					...
					if (parentPanel != this.panel)
					{
						...
						if (Mathf.Abs(vector3.x) > 0.001f || Mathf.Abs(vector3.y) > 0.001f)
						{
							...
							{
								"Panel can only be clipped properly if X and Y rotation is left at 0",
								this.panel
							});
						}
						angle = vector3.z;
