...
namespace ProtoBuf.Meta
{
	...
	{
		private sealed class DeserializeItemsIterator<T> : TypeModel.DeserializeItemsIterator, IDisposable, IEnumerator, IEnumerable, IEnumerator<T>, IEnumerable<T>
		{
		...
		protected internal static Type ResolveProxies(Type type)
		{
			...
			for (int i = 0; i < interfaces.Length; i++)
			{
				...
				if (fullName2 != null)
				{
                    //if (TypeModel.<>f__switch$map1 == null)
                    //{
                    //    TypeModel.<>f__switch$map1 = new Dictionary<string, int>(3)
                    //    {
                    //        {
                    ...
                    //}
                    //int num;
                    //if (TypeModel.<>f__switch$map1.TryGetValue(fullName2, out num))
                    //{
                    //    if (num == 0)
                    //    {
                    //        return type.BaseType;
