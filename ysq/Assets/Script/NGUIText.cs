...
public static class NGUIText
{
	...
	public static bool ParseSymbol(string text, ref int index, BetterList<Color> colors, bool premultiply, ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike, ref bool ignoreColor)
	{
		...
		if (text[index + 4] == ']')
		{
			...
			if (text3 != null)
			{
                //if (NGUIText.<>f__switch$map19 == null)
                //{
                //    NGUIText.<>f__switch$map19 = new Dictionary<string, int>(2)
                //    {
                //        {
                //            "[sub]",
                //            0
                ...
                //}
                //int num;
                //if (NGUIText.<>f__switch$map19.TryGetValue(text3, out num))
                //{
                //    if (num == 0)
                //    {
                //        sub = 1;
	...
	{
		return NGUIText.isSpace && (ch == 32 || ch == 8202 || ch == 8203 || ch == 8201);
	}
	...
	public static Vector2 CalculatePrintedSize(string text, int rgWidth)
	{
			...
			for (int i = 0; i < length; i++)
			{
				...
				else if (num4 >= 32)
				{
					...
					if (num5 != 0f)
					{
						...
						if (Mathf.RoundToInt(num + num5) > rgWidth)
						{
							if (num > num3)
							{
